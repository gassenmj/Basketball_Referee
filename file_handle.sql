create or replace package file_handle is 
 procedure ical_file_parser(
     p_name in varchar2
    ,p_activity_id in number
    ,p_person_licence_nr in varchar2
    ,p_team_id in number);
end file_handle;
/
      ...
      begin
            if p_pattern = c_location_pattern then 
             ...
             for i in l_var.first .. l_var.last loop
              logger.log(l_var(i),utl_call_stack.subprogram(1)(utl_call_stack.lexical_depth(1)+1));
             end loop;
            elsif p_pattern = c_teams_pattern then
             l_var := string2table(p_str,c_delimiter_loc);
             logger.log('Paarung: '||replace(l_var(2),'-',' vs. ')
             ...
             begin
              logger.log(to_char(to_date(p_str,'YYYYMMDD"T"hh24miss"Z"'))
                        ,utl_call_stack.subprogram(1)(utl_call_stack.lexical_depth(1)+1));
             exception when others then
              logger.log_error(p_scope=>utl_call_stack.subprogram(1)(utl_call_stack.lexical_depth(1)+1));
             end;
            end if;
           end make_it_fit;
    begin
           ...
           ;
            ...
            l_clob := translate( l_clob, chr(13)|| chr(10) || chr(9), c_delimiter||c_delimiter||c_delimiter ) ;
             ...
             loop
                     logger.log('--------------------',utl_call_stack.subprogram(1)(utl_call_stack.lexical_depth(1)+1));
